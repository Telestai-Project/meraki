...
/**
 * The Meowpow algorithm revision implemented as specified in the Meowpow spec
 * https://github.com/ethereum/wiki/wiki/Meowpow.
 */
...
struct meowpow_epoch_context
{
    ...
    const union meowpow_hash512* const light_cache;
 ...
 * by the Meowpow so the execution time is not constant. It takes ~ 0.01 ms.
 *
 ...
 *
 * This function will search for a prime number matching the criteria given
 * by the Meowpow so the execution time is not constant. It takes ~ 0.05 ms.
 *
 * @param epoch_number  The epoch number.
 ...
 */
union meowpow_hash256 meowpow_calculate_epoch_seed(int epoch_number) NOEXCEPT;
...
struct meowpow_result meowpow_hash(const struct meowpow_epoch_context* context,
    const union meowpow_hash256* header_hash, uint64_t nonce) NOEXCEPT;
...
bool meowpow_verify_final_hash(const union meowpow_hash256* header_hash,
