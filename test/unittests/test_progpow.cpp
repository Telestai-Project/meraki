...
TEST(progpow, l1_cache)
{
    ...
    for (size_t i = 0; i < cache_slice.size(); ++i)
        cache_slice[i] = meowpow::le::uint32(context.l1_cache[i]);
...
TEST(progpow, hash_30000)
{
    ...
    auto context = meowpow::create_epoch_context(meowpow::get_epoch_number(block_number));
...
{
    meowpow::epoch_context_ptr context{nullptr, nullptr};

    for (auto& t : progpow_hash_test_cases)
    {
        const auto epoch_number = meowpow::get_epoch_number(t.block_number);
        if (!context || context->epoch_number != epoch_number)
            context = meowpow::create_epoch_context(epoch_number);
...
TEST(progpow, search)
{
    auto ctxp = meowpow::create_epoch_context_full(0);
    auto& ctx = *ctxp;
    auto& ctxl = reinterpret_cast<const meowpow::epoch_context&>(ctx);
    ...
    EXPECT_EQ(sr.mix_hash, meowpow::hash256{});
    EXPECT_EQ(sr.final_hash, meowpow::hash256{});
    ...
    EXPECT_NE(sr.mix_hash, meowpow::hash256{});
    EXPECT_NE(sr.final_hash, meowpow::hash256{});
...
TEST(progpow, generate_hash_test_cases)
{
    ...
    {
        auto context = meowpow::create_epoch_context(e);
        auto block_numbers = {
            e * epoch_length,
