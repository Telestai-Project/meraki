...
TEST(progpow, l1_cache)
{
    ...
    for (size_t i = 0; i < cache_slice.size(); ++i)
        cache_slice[i] = meraki::le::uint32(context.l1_cache[i]);
...
TEST(progpow, hash_30000)
{
    ...
    auto context = meraki::create_epoch_context(meraki::get_epoch_number(block_number));
...
{
    meraki::epoch_context_ptr context{nullptr, nullptr};

    for (auto& t : progpow_hash_test_cases)
    {
        const auto epoch_number = meraki::get_epoch_number(t.block_number);
        if (!context || context->epoch_number != epoch_number)
            context = meraki::create_epoch_context(epoch_number);
...
TEST(progpow, search)
{
    auto ctxp = meraki::create_epoch_context_full(0);
    auto& ctx = *ctxp;
    auto& ctxl = reinterpret_cast<const meraki::epoch_context&>(ctx);
    ...
    EXPECT_EQ(sr.mix_hash, meraki::hash256{});
    EXPECT_EQ(sr.final_hash, meraki::hash256{});
    ...
    EXPECT_NE(sr.mix_hash, meraki::hash256{});
    EXPECT_NE(sr.final_hash, meraki::hash256{});
...
TEST(progpow, generate_hash_test_cases)
{
    ...
    {
        auto context = meraki::create_epoch_context(e);
        auto block_numbers = {
            e * epoch_length,
